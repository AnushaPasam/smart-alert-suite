...
export default function AnnouncementDetail({
  ...
  useEffect(() => {
    if (a && hasInitializedRef.current !== a.id) {
      hasInitializedRef.current = a.id;
      document.title = `${a.title} – EduAlert`;
      markAsRead(a.id);

      const viewedKey = `viewed_${a.id}`;
  ...
  const handleDownloadPDF = () => {
    import("jspdf").then(({ jsPDF }) => {
      ...
      doc.text(
        "Generated by EduAlert Announcement System",
        10,
        pageHeight - 10,
      );
      doc.text(
            ...
            <div className="relative z-10 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div
                    className={`h-14 w-14 sm:h-12 sm:w-12 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground shadow-lg shadow-primary/20 transition-transform ${isSpeaking && !isPaused ? "scale-110 animate-pulse" : ""}`}
                  >
                    <Volume2 className="h-7 w-7 sm:h-6 sm:w-6" />
                  </div>
                  <div>
                    <h4 className="text-base sm:text-sm font-black text-primary uppercase tracking-wider flex items-center gap-2">
                      Listen to Notice
                      {isSpeaking && (
                    ...
                    <p className="text-[11px] sm:text-[10px] text-muted-foreground font-bold mt-0.5 uppercase tracking-widest">
                      {isSpeaking
                        ? isPaused
                          ? "Paused"
                          : "Now Reading..."
                ...
                <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className={`h-11 sm:h-10 px-4 sm:px-0 sm:w-10 flex items-center justify-center rounded-xl border transition-all active:scale-95 shadow-sm ${isSettingsOpen ? "bg-primary text-primary-foreground border-primary" : "bg-card text-foreground border-border hover:border-primary/50"}`}
                    aria-label="Audio Settings"
                  >
                    <Settings2 className="h-5 w-5 sm:h-4 sm:w-4" />
                    <span className="sm:hidden ml-2 text-xs font-bold uppercase whitespace-nowrap">
                      Settings
                    </span>
                  </button>
                  <button
                    onClick={handleTTS}
                    className="h-11 sm:h-10 px-4 sm:px-0 sm:w-10 flex items-center justify-center bg-primary text-primary-foreground rounded-xl shadow-lg shadow-primary/20 hover:scale-105 transition-all active:scale-95"
                    aria-label={isSpeaking && !isPaused ? "Pause" : "Play"}
                  >
                    {isSpeaking && !isPaused ? (
                      <Pause className="h-5 w-5 sm:h-4 sm:w-4 fill-current" />
                    ) : (
                      <Play className="h-5 w-5 sm:h-4 sm:w-4 fill-current" />
                    )}
                    <span className="sm:hidden ml-2 text-xs font-bold uppercase whitespace-nowrap">
                  ...
                  {isSpeaking && (
                    <button
                      onClick={stopTTS}
                      className="h-11 sm:h-10 px-4 sm:px-0 sm:w-10 col-span-2 sm:col-auto flex items-center justify-center bg-destructive text-destructive-foreground rounded-xl shadow-lg shadow-destructive/20 hover:scale-105 transition-all active:scale-95 animate-fade-in"
                      aria-label="Stop"
                    >
              ...
              {isSettingsOpen && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 mt-4 bg-card/50 backdrop-blur-sm rounded-2xl border border-border/50 animate-in fade-in slide-in-from-top-2 relative">
                  <button
                    ...
                    <div className="flex p-1 bg-muted/50 rounded-lg border border-border">
                      {[1, 1.25, 1.5].map((rate) => (
                        <button
                          ...
                          className={`flex-1 py-1.5 sm:py-0.5 text-[10px] font-bold rounded-md transition-all ${playbackRate === rate ? "bg-card text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                        >
                          {rate}x
                        </button>
                      ))}
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full h-10 sm:h-8 px-2 text-[10px] font-bold bg-muted/50 border border-border rounded-lg outline-none focus:ring-1 focus:ring-primary/30 appearance-none"
                    >

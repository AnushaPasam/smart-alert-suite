...
export default function AnalyticsCharts({ announcements, categories }: { announcements: Announcement[]; categories: string[] }) {
  const categoryData = categories.map((c) => ({
    name: c,
    count: announcements.filter((a) => a.category === c).length,
  })).filter(d => d.count > 0);

  const mostViewed = [...announcements].sort((a, b) => b.views - a.views).slice(0, 5).map(a => ({
      ...
      <div className="campus-card-static p-5">
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={categoryData}>
            ...
            <Tooltip />
            <Bar dataKey="count" fill="hsl(216, 38%, 47%)" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="campus-card-static p-5 lg:col-span-2">
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={mostViewed} layout="vertical">
            ...
            <Tooltip />
            <Bar dataKey="views" fill="hsl(81, 21%, 45%)" radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>

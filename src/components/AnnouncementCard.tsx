...
export default function AnnouncementCard({ announcement: a, onToggleBookmark, basePath = "/student", compact = false }: Props) {
  ...
  return (
    <article
      ...
      className={`campus-card group ${compact ? "p-3 gap-2" : "p-5 gap-3"} flex flex-col relative cursor-pointer hover:shadow-md transition-all duration-200 ${!a.isRead ? "border-l-[3px] border-l-primary shadow-sm shadow-primary/5" : ""
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {!compact && (
          <>
            <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${priority.className}`}>
              {priority.label}
            </span>
      ...
      {/* Title */}
      <h3 className={`${compact ? "text-sm" : "text-base"} font-bold leading-snug group-hover:text-primary transition-colors line-clamp-2`}>
        {a.title}
      </h3>
      ...
      {/* Footer */}
      <div className={`flex items-center justify-between mt-auto ${compact ? "pt-1" : "pt-2 border-t border-border"} text-[10px] text-muted-foreground font-medium`}>

...
export default function About() {
  useEffect(() => {
    document.title = "About Us | EduAlert Announcement System";
  }, []);
    ...
    <div className="min-h-screen bg-background">
        ...
        <section className="grid lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground leading-relaxed">
              EduAlert is more than just an announcement system – it's a bridge
              between the administration and students. Our platform ensures that
              critical information reaches the right audience at the right time,
              fostering a transparent and connected academic environment.
            </p>
          ...
          {[
            {
              ...
              desc: "Designed with the student experience at the core of every feature.",
            },
            {
              icon: Shield,
              title: "Secure Data",
              desc: "Enterprise-grade security rituals to protect institutional data.",
            },
            {
              icon: Target,
              title: "Precision Reach",
              desc: "Smart filtering ensures relevant updates reach the targeted audience.",
            },
            {
              icon: Award,
              title: "Excellence",
              desc: "Committed to delivering high-performance communication solutions.",
            },
          ].map((item, i) => (
            <motion.div
              key={item.title}
        ...
        <section className="rounded-[3rem] bg-primary p-8 sm:p-16 text-center text-primary-foreground relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto space-y-6">
            <h2 className="text-3xl sm:text-5xl font-black">
              Ready to scale your campus community?
            </h2>
            <p className="text-primary-foreground/80 text-lg">
              Join hundreds of institutions already using EduAlert.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              <Link
                to="/user/register"
                className="px-10 py-4 bg-white text-primary font-bold rounded-2xl hover:bg-opacity-90 transition-all shadow-xl shadow-black/10"
              >
                Register Today
              </Link>
              <Link
                to="/contact"
                className="px-10 py-4 bg-primary-foreground/10 backdrop-blur-md text-white border border-white/20 font-bold rounded-2xl hover:bg-primary-foreground/20 transition-all"
              >
                Learn More
              </Link>
            </div>
      ...
      <footer className="py-12 border-t border-border mt-20 text-center">
        <p className="text-sm text-muted-foreground">
          © 2026 EduAlert Announcement System. All rights reserved.
        </p>
      </footer>
    </div>
  );

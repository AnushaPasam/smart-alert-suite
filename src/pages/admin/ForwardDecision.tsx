...
export default function AdminForwardDecision() {
  ...
  useEffect(() => { document.title = "Forward Decision – Smart Campus"; }, []);

  const approved = announcements.filter((a) => a.status === "Approved by Principal");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">{approved.length} approved announcements ready for publishing</p>
      </div>

      {approved.length === 0 ? (
        <div className="campus-card-static p-12 text-center">
          <CheckCircle className="h-10 w-10 mx-auto text-campus-olive mb-3" />
          <p className="text-muted-foreground">No approved announcements pending</p>
        </div>
      ) : (
            ...
            <div key={a.id} className="campus-card-static p-5 space-y-3">
              <div>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-campus-olive-light text-campus-olive font-medium">Approved ✓</span>
                </div>
              </div>
              <div className="flex gap-2 pt-2 border-t border-border">
                ...
                <button onClick={() => handleSchedule(a.id)}

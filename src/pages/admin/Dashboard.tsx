...
function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left campus-card p-5 flex items-start gap-4 transition-all duration-300 w-full ${
        isActive
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    document.title = "Admin Dashboard – EduAlert";
  }, []);

  // Filter announcements for Admin context: Sent to Principal and either Approved or Rejected
  const adminContextAnnouncements = announcements.filter(
    (a) =>
  ...
  const activeCount = adminContextAnnouncements.filter(
    (a) => a.status === "Published",
  ).length;
  const pendingPrincipalCount = adminContextAnnouncements.filter(
    (a) => a.status === "Pending Principal Approval",
  ).length;
  const rejectedCount = adminContextAnnouncements.filter(
    (a) => a.status === "Rejected",
  ).length;

  const filteredAnnouncements = adminContextAnnouncements.filter((a) => {
    if (activeTab === "Total") return true;
    if (activeTab === "Approved/Active")
      return a.status === "Published" || a.status === "Approved by Principal";
    if (activeTab === "Rejected") return a.status === "Rejected";
    if (activeTab === "Awaiting Principal")
      return a.status === "Pending Principal Approval";
    return true;
  });
  ...
  const stats = [
    {
      label: "Total",
      value: adminContextAnnouncements.length,
      icon: FileText,
    ...
    <div className="space-y-8 pb-12">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review and audit campus communication records
        </p>
      </div>

...
export default function AdminEdit() {
  ...
  const { announcements, updateAnnouncement, deleteAnnouncement, categories } =
    useAnnouncements();
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <form
            ...
            id="edit-form"
            className="campus-card-static p-8 space-y-6 shadow-xl"
          >
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-1 bg-muted/50 p-1 rounded-lg border border-border/50">
                  {(["high", "normal", "low"] as const).map((p) => (
                    <button
                      ...
                      className={`py-1.5 text-[10px] font-black uppercase tracking-tighter rounded-md transition-all ${
                        formData.priority === p
              ...
              <div className="pt-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  >
                    <div
                      className={`absolute top-0.5 h-3.5 w-3.5 rounded-full bg-white shadow-sm transition-transform ${formData.pinned ? "translate-x-5.5" : "translate-x-0.5"}`}
                    />
                  </div>
          ...
          <div className="p-5 rounded-2xl bg-primary/5 border border-primary/10 space-y-3">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-medium text-muted-foreground">
                Original Status:{" "}
                <span className="text-foreground font-bold">{a.status}</span>
              </p>
              <p className="text-[10px] font-medium text-muted-foreground">
                Created On:{" "}
                <span className="text-foreground font-bold">{a.createdAt}</span>
              </p>
              <p className="text-[10px] font-medium text-muted-foreground">
                Department:{" "}

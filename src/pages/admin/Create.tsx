...
export default function AdminCreate() {
  ...
  const handlePublish = (e: React.FormEvent) => {
    e.preventDefault();
    addAnnouncement({ title, description, category, expiryDate, priority, pinned });
    setPublished(true);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Compose and publish a new announcement</p>
      </div>
      ...
      <form onSubmit={handlePublish} className="campus-card-static p-6 space-y-5">
        <div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-end">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              >
                <div className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow-sm transition-transform ${
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-border rounded-lg p-6 text-center cursor-pointer hover:bg-muted/50 transition-colors">
            ...
            <p className="text-xs text-muted-foreground mt-1">PDF, DOC, PNG up to 10MB</p>
          </div>
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {[
          { icon: Sparkles, title: "AI Category Suggestion", desc: "Based on content analysis, suggested category: Academic" },
          { icon: AlertTriangle, title: "AI Priority Prediction", desc: "Predicted priority based on keywords: Normal" },
          { icon: Clock, title: "Deadline Reminder", desc: "Reminder will be sent 2 days before expiry" },
          { icon: CalendarClock, title: "Schedule Extractor", desc: "Detected dates: March 15, 2026 (exam start)" },
        ].map((ai) => (

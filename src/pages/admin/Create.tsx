...
export default function AdminCreate() {
  ...
  useEffect(() => {
    document.title = "Create Announcement – Smart Campus";
  }, []);
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            >
              <div
                className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow-sm transition-transform ${pinned ? "translate-x-5" : "translate-x-0.5"}`}
              />
            </div>
        ...
        {[
          {
            ...
            desc: "Based on content analysis, suggested category: Academic",
          },
          {
            icon: AlertTriangle,
            title: "AI Priority Prediction",
            desc: "Predicted priority based on keywords: Normal",
          },
          {
            icon: Clock,
            title: "Deadline Reminder",
            desc: "Reminder will be sent 2 days before expiry",
          },
          {
            icon: CalendarClock,
            title: "Schedule Extractor",
            desc: "Detected dates: March 15, 2026 (exam start)",
          },
        ].map((ai) => (
          <div
            key={ai.title}
            className="campus-card-static p-4 flex items-start gap-3"
          >

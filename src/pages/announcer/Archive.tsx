...
export default function AnnouncerArchive() {
  ...
  useEffect(() => {
    document.title = "Announcement Archive – Smart Campus";
  }, []);

  const archived = (announcements || [])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Old announcements stored here for 60 days before automatic deletion.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {archived.length === 0 ? (
                <tr>
                  ...
                  >
                    No archived announcements at this time.
                  </td>
                </tr>
              ) : (
                archived.map((a) => {
                  const archivedAtDate = a.archivedAt
                  ...
                  return (
                    <tr
                      key={a.id}
                      className="hover:bg-muted/30 transition-colors opacity-75"
                    >

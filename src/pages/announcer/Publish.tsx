...
export default function AnnouncerPublish() {
  ...
  useEffect(() => { document.title = "Publish Announcements – Smart Campus"; }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">{approved.length} approved announcements ready to publish</p>
      </div>

      {approved.length === 0 ? (
        <div className="campus-card-static p-12 text-center">
          <Send className="h-10 w-10 mx-auto text-muted-foreground/40 mb-3" />
          <p className="text-muted-foreground">No approved announcements to publish</p>
        </div>
      ) : (
          ...
          {approved.map((a) => (
            <div key={a.id} className="campus-card-static p-5 space-y-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{a.category} · {a.department}</p>
              </div>

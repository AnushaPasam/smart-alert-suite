...
export default function AnnouncerStatusHistory() {
  ...
  useEffect(() => {
    document.title = "Submission History – Smart Campus";
  }, []);

  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Published":
        return <CheckCircle className="h-4 w-4 text-campus-olive" />;
      case "Rejected":
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "Rejected":
        return "bg-priority-high/10 text-destructive border border-destructive/20";
      case "Pending Admin Approval":
      case "Pending Principal Approval":
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Track the status and progress of your announcement requests
        </p>
      </div>
            ...
            <tbody className="divide-y divide-border">
              {announcements.length === 0 ? (
                <tr>
                  ...
                  >
                    You haven't submitted any announcements yet.
                  </td>
                </tr>
              ) : (
                announcements.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-center">
                        <div
                          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${getStatusBadge(a.status)}`}
                        >
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <Link
                          to={`/announcer/announcement/${a.id}`}
                          className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-primary transition-colors"
                          title="View Announcement Content"
                        >

...
export default function UserDashboard() {
  ...
  useEffect(() => { document.title = "Dashboard – Smart Campus"; }, []);

  // Only show published announcements to users, auto-archive expired
  const published = announcements.filter((a) => a.status === "Published");
  const now = new Date();
  const active = published.filter((a) => new Date(a.expiryDate) >= now);
  const sorted = sortAnnouncements(active);
  ...
  useEffect(() => { setVisibleCount(BATCH_SIZE); }, [search, categoryFilter]);

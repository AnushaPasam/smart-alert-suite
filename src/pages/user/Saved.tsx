...
export default function UserSaved() {
  ...
  useEffect(() => {
    document.title = "Saved – Smart Campus";
  }, []);
  ...
  const cb = useCallback(
    (entries: IntersectionObserverEntry[]) => {
      if (entries[0].isIntersecting && hasMore && !loading) {
        setLoading(true);
        setTimeout(() => {
          setCount((p) => Math.min(p + BATCH, bookmarked.length));
          setLoading(false);
        }, 400);
      }
    },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {bookmarked.length} saved announcements
        </p>
      </div>
      ...
      {bookmarked.length === 0 ? (
        <div className="text-center py-16 campus-card-static">
          ...
          <p className="text-muted-foreground">No saved announcements</p>
          <p className="text-xs text-muted-foreground mt-1">
            Bookmark announcements to save them here
          </p>
        </div>
      ) : (
        <>

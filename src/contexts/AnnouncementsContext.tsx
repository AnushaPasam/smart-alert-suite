...
interface AnnouncementsContextType {
  ...
  addAnnouncement: (
    a: Omit<
      Announcement,
      "id" | "views" | "isRead" | "isBookmarked" | "createdAt" | "comments"
    >,
  ) => void;
...
export function AnnouncementsProvider({ children }: { children: ReactNode }) {
  const [announcements, setAnnouncements] =
    useState<Announcement[]>(dummyAnnouncements);
  ...
  const updateStatus = useCallback(
    (id: string, status: AnnouncementStatus, rejectionReason?: string) => {
      setAnnouncements((prev) =>
        prev.map((a) =>
          a.id === id
            ? {
  ...
  const addComment = useCallback(
    (announcementId: string, comment: Omit<Comment, "id">) => {
      setAnnouncements((prev) =>
        prev.map((a) => {
